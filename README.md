# 故事生成器
这是我花了两天时间从零用ai写的小游戏，主要是没在市面上看到类似的小游戏，不是要钱，就是太复杂（sillytavern）
代码写得还比较粗糙，有很多可以优化的地方，也可能存在一些 bug，
毕竟是第一次做，而且我也不怎么懂编程。

这是一款基于 Python 的交互式故事生成器。玩家设定角色，通过文字行动；AI 根据世界状态、时间和预设事件节点生成剧情。世界以"天+时段"推进，节点事件强制发生。
